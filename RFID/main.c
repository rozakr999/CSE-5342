...
// Sending bytes in 'tx' and receive into 'rx'.
                                  ...
                                  uint8_t bitFraming)
{
    ...
    for (uint8_t i = 0; i < txLen; i++)
        mfrc522_write(FIFODataReg, tx[i]);
...
static void mfrc522_crc(const uint8_t *data, uint8_t len, uint8_t out[2])
{
    ...
    for (uint8_t i = 0; i < len; i++)
        mfrc522_write(FIFODataReg, data[i]);
    mfrc522_write(CommandReg, PCD_CalcCRC);

    // wait CRCIRq
    for (uint16_t t = 0; t < 5000; t++)
    {
        if (mfrc522_read(DivIrqReg) & 0x04)
            break;
    }
...
static uint8_t picc_anticoll_cl1(uint8_t uid[4])
{
    ...
    uid[3] = rx[3];
    uint8_t bcc = rx[4];
    if ((uid[0] ^ uid[1] ^ uid[2] ^ uid[3] ^ bcc) != 0)
        return 5; // BCC fail
    return 0;
}
...
static const char EXPECTED_PIN[4] = {'2', '5', '8', '0'}; // pin to match
...
static uint8_t check_pin_blocking(uint32_t timeout_ms)
{
    ...
    if (n != 4)
        return 0; // timed out or incomplete
