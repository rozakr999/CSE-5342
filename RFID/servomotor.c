...
//PWM setup
//PA9 as PWM output using TIM1_CH2
...
void initPwm(void)
{
	...
	//PA9 as alternate function
	PWM_GPIO_PORT->MODER &= ~(3U << (2*PWM_GPIO_PIN)); //clear mode bits
	...
	//configure channel 2: pwm mode 1, preload enable
...
{
	//keep pulse width in safe range
...
{
	//sweep servo 75 -> 0
		for (uint8_t angle = 75; angle > 0; angle -= 5) // angle per step; lower number for slower sweep
		{
			setServoAngle(angle);
		...
		//sweep back 0 -> 75
		for (uint8_t angle = 0; angle <= 75; angle +=5)
		{
			setServoAngle(angle);
			delay_Ms(50);
		}
